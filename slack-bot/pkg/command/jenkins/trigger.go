...
func (c *triggerCommand) genericCall(match matcher.Result, message msg.Message) {
	...
	err = jenkins.TriggerJenkinsJob(jobConfig.config, jobName, finalParameters, c.SlackClient, c.jenkins, message, c.DB)
	if err != nil {
		c.ReplyError(message, err)
		return
	}
...
func (c *triggerCommand) configTrigger(ref msg.Ref, text string) bool {
	...
	for jobName, jobConfig := range c.jobs {
		...
		err = jenkins.TriggerJenkinsJob(jobConfig.config, jobName, jobParams, c.SlackClient, c.jenkins, ref.WithText(text), c.DB)
		if err != nil {
			c.ReplyError(ref, err)
		}
